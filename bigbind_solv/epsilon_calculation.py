...
class SolvDatasetReporterWithCustomDP(SolvDatasetReporter):
    ...
    def report(self, simulation, state):
        ...
        sterics_derivative = self.get_parameter_derivative(simulation, LAMBDA_STERICS)
        electrostatics_derivative = self.get_parameter_derivative(simulation, LAMBDA_ELECTROSTATICS)
        ...
        vac_electrostatics_derivative = self.get_parameter_derivative(self.system_vac.simulation, LAMBDA_ELECTROSTATICS)
...
def runSim(base_file_path, sim_file_path, steps, dp, lambda_electrostatics, lambda_sterics):
    ...
    simulation.reporters.append(DCDReporter(os.path.join(sim_file_path, "simulation.dcd"), 100))
...
def randomSample(bigbind_path):
    if os.path.exists(bigbind_path):
        folders = [f for f in glob.glob(os.path.join(bigbind_path, "[0-9]*")) if os.path.isdir(f)]
...
def statsAnalysis(base_path, folder_paths):
    ...
    for (lambda_elec, dp) in com:
        ...
        for folder in folder_paths:
            ...
            sim_path = os.path.join(base_path, folder, f"Epsilon_{dp}_{lambda_elec}", "sim1M.h5")
            print(sim_path)
        ...
        print(f"Current DP: {dp}; Current Lambda_Elec {lambda_elec} STD: {change}; Average {avg}")
