...
def runAll(folder, smile, name, start, end):
    ...
    system = get_lr_complex(None, sdf_file,
                solvent="tip3p",
                nonbonded_method=app.PME,
                include_barostat=True,
                **kwargs
    )
    system_vac = get_lr_complex(None, sdf_file,
                solvent="none",
                **kwargs
    )
    ...
    if(not os.path.exists(os.path.join(folder, 'init.dcd'))):
        ...
        simulation = app.Simulation(system.topology, system.system, integrator, system.platform)
    ...
    for idx, coords in tqdm(enumerate(traj.xyz), total=len(traj.xyz), desc="Processing trajectory"):
        ...
        try:
            ...
            set_fep_lambdas(alchemical_system.simulation.context, lambda_sterics, lambda_electrostatics)
            set_fep_lambdas(alchemical_system_vac.simulation.context, lambda_sterics, lambda_electrostatics)
            
            vac_coords = np.array(coords)[vac_indicies]
            alchemical_system_vac.set_positions(vac_coords.tolist()*unit.nanometer)
            
            solv_reporter = SolvDatasetReporterWithCustomDP(sim_path, alchemical_system_vac, 500, dp)
