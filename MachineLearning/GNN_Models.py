    ...
    def forward(self,
        ...
        if self.gnn_params is not None:
            x = torch.cat([
                ...
                torch.full((positions.detach().shape[0], 1), lambda_electrostatics.item(), device=self._device),
                torch.full((positions.detach().shape[0], 1), lambda_sterics.item(), device=self._device)
                ],
                dim=-1)
