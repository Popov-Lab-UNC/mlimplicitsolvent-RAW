...
class Trainer:
    ...
    def indentify_problematic_entries(
        self, batch_size=128, loss_limit=100000, save_path=None, verbose=False
    ):
        """
        ...
        for i, data in enumerate(self._validation_data):
            ...
            for l, ldata in tqdm.tqdm(enumerate(loader)):
                ...
                loss = calculate_force_loss_max_individual_for_pre_in_range(
                    pre_energy, pre_forces, ldata
                )
    ...
    def test_model(self, batch_size=32, return_predictions=False):
        ...
        for i, data in enumerate(
            Data_Holder(self._training_data[:] + self._validation_data[:])
        ):
            loader = DataLoader(data, batch_size=batch_size)
    ...
    def get_gbneck2_param(pdb_id, work_dir, uniqueRadii=None, cache=None):
        ...
        if "_in_" in pdb_id:
            return Trainer.get_gbneck2_param_small_molecules(
                pdb_id, work_dir, cache=cache
            )
    ...
    def prepare_training_data_for_multiple_peptides_from_file(
        self, folder_path, train_test_split=0.8, work_dir="."
    ):
        ...
        for file in [file for file in files if "force_out.txt.npy" in file]:
            filecount += 1
            pid = file.split("_")[0]
            atom_features, uniqueRadii = self.get_gbneck2_param(
                pid, work_dir, uniqueRadii
            )
            ...
            data = [
                Data(
                    pos=torch.tensor(
                        coord_set[i], dtype=torch.float, requires_grad=True
                    ),
                    energies=torch.tensor(
                        [force_energy_set[i][0, 0]], dtype=torch.float
                    ).unsqueeze(1),
                    forces=torch.tensor(force_energy_set[i][:, 1:], dtype=torch.float),
                    atom_features=torch.tensor(atom_features, dtype=torch.float),
                )
                for i in range(len(coord_set))
                if force_energy_set[i][:, 1:][0, 0] != 0
            ]
            print(filecount, len(data))
            Dataset += data

        print(
            "Constructed Dataset with %i frames from %i compound sets"
            % (len(Dataset), len(files) / 3)
        )

        training_data, validation_data, test_data = self.do_split(
            Dataset, 1 - train_test_split
        )
    ...
    def prepare_training_data_for_multiple_peptides(
        self, peptide_id_list=[], train_test_split=0.8, work_dir="."
    ):
            ...
            for file in [file for file in files if "force_out.txt.npy" in file]:
                ...
                data = [
                    Data(
                        ...
                        forces=torch.tensor(
                            force_energy_set[i][:, 1:], dtype=torch.float
                        ),
                        atom_features=torch.tensor(atom_features, dtype=torch.float),
                    )
                    for i in range(len(coord_set))
        ...
        print(
            "Constructed Dataset with %i frames from %i compound sets"
            % (len(Dataset), len(peptide_id_list))
        )
    ...
    def prepare_training_data_from_query_pairs(
        self,
        query_results,
        train_test_split=0.8,
        work_dir=".",
        save_full_dataset_path_to_file=None,
    ):
        """
        ...
        for positions, forces, atomfeatures in query_results:
            ...
            atom_features = self.reset_gbn2_parameters_for_radii(
                np.array(atomfeatures), tot_unique
            )
            atom_features = torch.tensor(atom_features, dtype=torch.float)
            ...
            data = [
                Data(
                    pos=pos,
                    energies=force[0, 0].unsqueeze(0).unsqueeze(1),
                    forces=force[:, 1:],
    ...
    @staticmethod
    def get_gbneck2_param_small_molecules_unique(
        pid, work_dir, uniqueRadii=None, pdb=None, cache=None, num_it=10
    ):

        sim = Simulator(
            work_dir=work_dir, pdb_id=pid, run_name="getparam", solute_pdb=pdb
        )
        sim.forcefield = OpenFF_forcefield_GBNeck2(pid, cache=cache)
        topology = sim._datahandler.topology
        charges = np.array(
    ...
    def reset_gbn2_parameters_for_radii(gbn2_parameters, uniqueRadii):
        ...
        radiusToIndex = {r: i for i, r in enumerate(uniqueRadii)}
        gbn2_parameters[:, 6] = [
            radiusToIndex[float("%.4f" % (r + offset))] for r in gbn2_parameters[:, 1]
        ]

        return gbn2_parameters

    def prepare_training_data_for_multiple_small_molecules_folder(
        self, folder_path, train_test_split=0.8, work_dir=".", mapping_file=None
    ):
        ...
        for file in [file for file in files if "force_out.txt.npy" in file]:
            ...
            atom_features, uniqueRadii = self.get_gbneck2_param_small_molecules_unique(
                solute + "_in_v", work_dir
            )
            all_u += uniqueRadii
        uniqueRadii = list(sorted(set(all_u)))
        for file in [file for file in files if "force_out.txt.npy" in file]:
            ...
            if "_id_" in file:
                ...
                solute = pid.split("_in_")[0]
                solvent = pid.split("_in_")[1]
            atom_features, _ = self.get_gbneck2_param_small_molecules_unique(
                solute + "_in_v", work_dir, uniqueRadii
            )
            unique_gbneckparam.append(atom_features)
            core = file.split("force")[0]
            coord_set = np.load(folder_path + core + "pos_out.txt.npy")
            ...
            data = [
                Data(
                    ...
                    forces=torch.tensor(force_energy_set[i][:, 1:], dtype=torch.float),
                    atom_features=torch.tensor(atom_features, dtype=torch.float),
                    solvent_epsilon=torch.tensor(
                        [SOLVENT_EDICT[solvent] for i in range(len(atom_features))],
                        dtype=torch.float,
                    ),
                    solventIndex=torch.tensor(
                        [SOLVENT_IDICT[solvent] for i in range(len(atom_features))],
                        dtype=torch.long,
                    ),
                )
                for i in range(len(coord_set))
        ...
        print(
            ...
            % (
                len(Dataset),
                len([file for file in files if "force_out.txt.npy" in file]),
            )
        )
    ...
    def prepare_training_data_for_multiple_small_molecules(
        self, peptide_id_list=[], train_test_split=0.8, work_dir="."
    ):
        print("DEPRICATED")
        ...
        for pid in peptide_id_list:
            ...
            atom_features = self.get_gbneck2_param_small_molecules(
                solute + "_in_v", work_dir
            )
            unique_gbneckparam.append(atom_features)
    ...
    def prepare_training_data_from_multiple_small_molecules_from_preprocessed_storage(
        self,
        hdf5_list=[],
        train_test_split=0.8,
        work_dir=".",
        ...
        for hdf5_file in tqdm.tqdm(hdf5_list):
            ...
            try:
                ...
                (
                    ...
                    confids_l,
                    hdf5s_l,
                ) = hs.get_entire_training_set(reextracted_forces=use_reextracted)
                for coo, fo, at, un, sm, mo, con, hd in zip(
                    coords_l,
                    forces_l,
                    atom_features_l,
                    uniquer_l,
                    ...
                    if (fo.shape[1] == at.shape[0]) and (coo.shape[1] == fo.shape[1]):
                        ...
                        data = [
                            Data(
                                pos=torch.tensor(
                                    coo[i], dtype=torch.float, requires_grad=True
                                ),
                                energies=torch.tensor(
                                    [fo[i][0, 0]], dtype=torch.float
                                ).unsqueeze(1),
                                forces=torch.tensor(fo[i][:, 1:], dtype=torch.float),
                                atom_features=torch.tensor(at, dtype=torch.float),
                                solvent_epsilon=torch.tensor(
                                    [SOLVENT_EDICT[solvent] for j in range(len(at))],
                                    dtype=torch.float,
                                ),
                                solventIndex=torch.tensor(
                                    [SOLVENT_IDICT[solvent] for j in range(len(at))],
                                    dtype=torch.long,
                                ),
                                smiles=sm,
                                molid=mo,
    ...
    def get_forces_of_storage(self, hdf5_file, molid, confid, force_id, solvent="O"):
        ...
        for coo, fo, at, un, sm, mo, con, hd in zip(
            ...
            if (mo == molid) and (con == confid):
                ...
                at = self.reset_gbn2_parameters_for_radii(at, tot_unique)
                data = Data(
                    pos=torch.tensor(coo[i], dtype=torch.float, requires_grad=True),
                    energies=torch.tensor([fo[i][0, 0]], dtype=torch.float).unsqueeze(
                        1
                    ),
                    forces=torch.tensor(fo[i][:, 1:], dtype=torch.float),
                    atom_features=torch.tensor(at, dtype=torch.float),
                    ...
                    confid=con,
                    hdf5=hd,
                    batch=torch.zeros(len(at), dtype=torch.long),
                )

        return self._model(data.to("cuda"))

    def prepare_training_data_from_pt_file(
        self,
        pt_file,
        np_file=None,
        train_test_split=0.8,
    ...
    def prepare_training_data_for_multiple_small_molecules_from_storage(
        self, hdf5_list=[], train_test_split=0.8, work_dir=".", solvent="O"
    ):
            ...
            try:
                ...
                for mol_id in m_c_dict.keys():
                    for conf_id in m_c_dict[mol_id]:
                        ...
                        try:
                            atom_features, uniqueRadii = (
                                storage.get_atom_features_and_unique_radii(
                                    mol_id, conf_id
                                )
                            )
                        except:
                        ...
                        if isinstance(atom_features, bool):
                            ...
                            if solute != pref_solute:
                                atom_features, uniqueRadii = (
                                    self.get_gbneck2_param_small_molecules_unique(
                                        solute + "_in_v", work_dir, pdb=pdb
                                    )
                                )
                            else:
                    ...
                    for conf_id in m_c_dict[mol_id]:
                        ...
                        if isinstance(atom_features, bool):
                            ...
                            if solute != pref_solute:
                                atom_features, single_unique = (
                                    self.get_gbneck2_param_small_molecules_unique(
                                        solute + "_in_v", work_dir, uniqueRadii, pdb=pdb
                                    )
                                )
                            else:
                                atom_features = pref_at
                                single_unique = pref_un
                            storage.create_atom_feature_entry(
                                mol_id, conf_id, atom_features, single_unique
                            )
                        ...
                        unique_gbneckparam.append(atom_features)
                        force_energy_set, coord_set, frames = storage.get_extraction(
                            mol_id, conf_id
                        )
                        if (force_energy_set.shape[1] == atom_features.shape[0]) and (
                            coord_set.shape[1] == force_energy_set.shape[1]
                        ):
                            ...
                            data = [
                                Data(
                                    ...
                                    atom_features=torch.tensor(
                                        atom_features, dtype=torch.float
                                    ),
                                    solvent_epsilon=torch.tensor(
                                        [
                                            SOLVENT_EDICT[solvent]
                                            for j in range(len(atom_features))
                                        ],
                                        dtype=torch.float,
                                    ),
                                    solventIndex=torch.tensor(
                                        [
                                            SOLVENT_IDICT[solvent]
                                            for j in range(len(atom_features))
                                        ],
                                        dtype=torch.long,
                                    ),
    ...
    def preprocess_single_storage_and_store_data(
        self, hdf5_file, train_test_split=0.8, work_dir=".", solvent="O"
    ):
        ...
        for mol_id in m_c_dict.keys():
            for conf_id in m_c_dict[mol_id]:
                ...
                try:
                    atom_features, single_unique = (
                        storage.get_atom_features_and_unique_radii(mol_id, conf_id)
                    )
                except:
                    # print('no forces for %s %s' % (mol_id,conf_id))
                    continue
    ...
    def prepare_training_data(self, train_test_split=0.8):
        ...
        training_from_to, val_from_to, test_from_to = self.do_split(
            from_to_pairlist, 1 - train_test_split
        )
        ...
        self._training_data = Data_Iterator(
            training_from_to, len(training_from_to), data_preparation_function
        )
        self._validation_data = Data_Iterator(
            val_from_to, len(val_from_to), data_preparation_function
        )
        if not test_from_to is None:
            self._test_data = Data_Iterator(
                test_from_to, len(test_from_to), data_preparation_function
            )
    ...
    def do_split(self, pairlist, split=0.2):
        ...
        if split != 0.0:
            train_idx, test_idx = train_test_split(
                np.arange(len(pairlist)),
                test_size=split,
                random_state=self._random_state,
            )
            return (
                [pairlist[i] for i in train_idx],
                [pairlist[i] for i in test_idx],
                None,
            )
        else:
        ...
        for fr_to in from_to_pair:
            ...
            data = [
                Data(
                    ...
                    forces=torch.tensor(force_energy_set[i][:, 1:], dtype=torch.float),
                    atomic_features=torch.tensor(atom_features, dtype=torch.float),
                )
                for i in range(len(coord_set))
            ]
            Dataset += data
            ...
            if force_energy_set[0][1, 0] == 0:
                ...
                data = [
                    Data(
                        ...
                        energies=torch.tensor(
                            force_energy_set[i][:, 0], dtype=torch.float
                        ).unsqueeze(1)
                        * -1,
                        forces=torch.tensor(
                            force_energy_set[i][:, 1:], dtype=torch.float
    ...
    def get_graphs_from_files(self, from_to_pair):
        ...
        for fr_to in from_to_pair:
            ...
            graphs = [
                get_Graph_for_one_frame(
                    coord, protein_set[:, 11:13].astype(float), y=radii_set[i, :]
                )
    ...
    def check_training_files(self, files):
        ...
        if self.explicit:
            ...
            exist_from_to = [
                from_to.split(".")[0]
                for from_to in coord_files
                if from_to.split("pos_out")[0] + "force_out.txt.npy"
                in force_energy_files
            ]
            print(exist_from_to)
            from_to = np.array(
                [
                    [from_to.split("_")[0], from_to.split("_")[1].split("pos")[0]]
                    for from_to in exist_from_to
                ]
            )
            ...
            if self._force_mode:
                ...
                exist_from_to = [
                    from_to.split(".")[0]
                    for from_to in coord_files
                    if from_to.split(".")[0] + "_out.txt.npy" in radii_files
                ]
                ...
                exist_from_to = [
                    from_to.split(".")[0]
                    for from_to in coord_files
                    if from_to.split(".")[0] + "_out.txt" in radii_files
                ]

            # Get data as from to array
            from_to = np.array(
                [
                    [from_to.split("_")[0], from_to.split("_")[1]]
                    for from_to in exist_from_to
                ]
            )
            from_to = from_to.astype(int)
        ...
        if self._verbose:
            print(
                "Dataset consists of %i files with %i frames resulting in %i total frames"
                % (
                    from_to.shape[0],
                    from_to[0, 1] - from_to[0, 0] + 1,
    ...
    def get_from_to(self, fr, to):
        ...
        if self.explicit:
            if self._use_tmpdir:
                coord_path = (
                    self._tmp_folder + str(fr) + "_" + str(to) + "pos_out.txt.npy"
                )
                force_energy_path = (
                    self._tmp_folder + str(fr) + "_" + str(to) + "force_out.txt.npy"
                )
                proteinpath = self._tmp_folder + "Protein_Info.csv"
            else:
                coord_path = (
                    self._data_path + str(fr) + "_" + str(to) + "pos_out.txt.npy"
                )
                force_energy_path = (
                    self._data_path + str(fr) + "_" + str(to) + "force_out.txt.npy"
                )
            ...
            if self._use_tmpdir:
                ...
                if self._force_mode:
                    force_energy_path = (
                        self._tmp_folder + str(fr) + "_" + str(to) + "_out.txt.npy"
                    )
                else:
                    force_energy_path = (
                        self._tmp_folder + str(fr) + "_" + str(to) + "_out.txt"
                    )
                proteinpath = self._tmp_folder + "Protein_Info.csv"
            else:
                ...
                if self._force_mode:
                    force_energy_path = (
                        self._data_path + str(fr) + "_" + str(to) + "_out.txt.npy"
                    )
                else:
                    force_energy_path = (
                        self._data_path + str(fr) + "_" + str(to) + "_out.txt"
                    )
                proteinpath = self._data_path + "Protein_Info.csv"
    ...
    def load_model_dict_for_finetuning(self, model_path):
        ...
        pretrained_dict = {
            ...
            if (
                ("aggregate_information" not in k)
                and ("_d0" not in k)
                and ("_m0" not in k)
            )
        }
    ...
    def plot_train_v_predicted_pos(
        self, return_forces=False, max_s=None, fromtopairs=[[0, 10]]
    ):
        ...
        for data in self._validation_data:
            ...
            for l, ldata in enumerate(loader):
                ...
                for b in torch.unique(ldata.batch):
                    dism = torch.cdist(
                        ldata.pos[ldata.batch == b], ldata.pos[ldata.batch == b]
                    )
                    tmpdis.append(
                        [dism[pair[0], pair[1]].tolist() for pair in fromtopairs]
                    )
    ...
    def prepare_run_model(
        ...
        pretrained_dict = {
            k: v for k, v in pretrained_dict.items() if "aggregate_information" not in k
        }
        ...
        if savename is None:
            savename = (
                self._model_path.split("/")[-1].split("model.model")[0]
                + "_run_%s.pt" % pdbid
            )
        torch.jit.optimize_for_inference(torch.jit.script(gnn_run.eval())).save(
            savedir + savename
        )
...
class Evaluator(Trainer):
    ...
    def __init__(
        self, name="name", path=".", verbose=True, enable_tmp_dir=True, force_mode=False
    ):
        super().__init__(
            name=name,
            path=path,
    ...
    def get_validation_data(self):
        if self._model_path is not None:
            ...
            warnings.warn(
                "Model Path was not provided; please use Evaluator.model = str"
            )
    ...
    def plot_val_for_frame(self, val_idx, frame_idx, save_txt=""):
        ...
        plt.text(
            0.3, 0.75, "RMSE: %.4f" % np.sqrt(mean_squared_error(tru, pre)), fontsize=16
        )
        if save_txt != "":
