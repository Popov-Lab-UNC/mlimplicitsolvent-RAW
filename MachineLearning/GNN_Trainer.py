...
class Trainer:
    ...
    def train_model(self, runs, batch_size=100, clip_gradients = 0,):
        assert self._optimizer != None
        assert self._model != None
        self.wandblog(self._name, batch_size, clip_gradients, runs)
        self._model.train()
        Val_Losses = np.empty((runs))
        loader = DataLoader(self._training_data, batch_size = batch_size, num_workers = 4,shuffle=True, pin_memory = True)    
        ...
        for i in range(runs):
            ...
            total_loss = []
            Val_Losses = []
            for d, ldata in enumerate(tqdm(loader)):
                # set optimizer gradients
                self._optimizer.zero_grad()
                ...
                if self._verbose:
                    ...
                    d, (time.time() - start) / (d + 1), np.mean(loss.to('cpu').tolist())), end="\r")
    ...
    def identify_problematic_sets(self,error_tolerance=5000):
        ...
        for i, data in enumerate(self._validation_data):
            ...
            for l, ldata in enumerate(loader):
                ldata.to(self._device)
                pre_energy, pre_forces = self._model(ldata)
                loss = self.calculate_loss(pre_energy, pre_forces, pre_sterics, pre_electrostatics, ldata)
    ...
    def validate_model(self,batch_size=8):
