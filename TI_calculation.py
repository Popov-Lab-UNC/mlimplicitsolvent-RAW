...
class AI_Solvation_calc_TI:
    ...
    def set_system(self, lambda_electrostatics):
        self.system, self.molecule, self.topology = self.create_system_from_smiles(self.smiles, self.pdb_path)
        print(" -- Saving Partial Charges -- ")
        nonbonded = [f for f in self.system.getForces() if isinstance(f, NonbondedForce)][0]
        self.charges =  np.array([tuple(nonbonded.getParticleParameters(idx))[0].value_in_unit(elementary_charge) for idx in range(self.system.getNumParticles())])
    ...
    def run_all_sims(self, overwrite = False):
        ...
        if not os.path.exists(self.pdb_path):
            ...
            self.savePDB(self.pdb_path) #ensure current system does not have changed electrostatics
        ...
        for lambda_elec in reversed(self.lambda_electrostatics):
            self.AI_simulation(1.0, lambda_elec, vaccum = 0.0, out = f"(1.0-{lambda_elec})_{self.name}")
        solv_elec_time = time.time()
        print(f" -- Time taken: {solv_elec_time - setup_time} - Removing Sterics -- ")
        for lambda_ster in reversed(self.lambda_sterics):
            self.AI_simulation(lambda_ster, 0.0, vaccum = 0.0, out = f"({lambda_ster}-0.0)_{self.name}")
        solv_ster_time = time.time()

        print(f" -- Finished Simulation -- Total Time: {time.time() - start} -- ")
    ...
    def collateInfo(self):
        ...
        for (lambda_ster, lambda_elec) in tqdm(self.get_solv_lambda_schedule()):
            ...
            derivatives.append(self.meanCalculation(traj, lambda_ster, lambda_elec))
        print(derivatives)
        ...
        u_elec_der = np.array([np.median(np_elec_der[np_elec == ux]) for ux in u_elec])
        u_ster_der = np.array([np.median(np_ster_der[np_ster == ux]) for ux in u_ster])
